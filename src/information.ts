 ...
 * servers must not send a 1xx response to an HTTP/1.0 compliant 
...
export enum Information {
	...
	*	for example, a POST request). Sending a large request body to a server after a 
	*	request has been rejected for inappropriate headers would be inefficient. 
	...
	* that the request should be repeated without the Expect header as it indicates that 
...
   */
	 ...
	 * but no response is available yet. This prevents the client from timing out and 
